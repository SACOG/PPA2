...
Purpose: Get data for each TMC for PPA 2.0 calcs:
	...
	Road number,
	F_System,
	off-peak 85th percentile (85th pctl for fwys; for arterials is 60th pctl to account for signal delay) speed (8pm-6am, all days),
	80th percentile TT:
		Weekdays 6am-10am
		Weekdays 10am-4pm
		Weekdays 4pm-8pm
...
WHERE (DATEPART(hh,measurement_tstamp) >= @FFprdStart
		...
		AND tt.tmc_code = '105-16905'

select * from #ff_spd_tbl
...
WHERE DATENAME(dw, tt.measurement_tstamp) IN (SELECT day_name FROM @weekdays) 
	AND avs.hour_cong_rank_70thffs < 5
...
FROM npmrds_2018_alltmc_paxtruck_comb tt 
	JOIN #avspd_x_tmc_hour avs
		ON tt.tmc_code = avs.tmc_code
		AND DATEPART(hh, tt.measurement_tstamp) = avs.hour_of_day
WHERE avs.hour_cong_rank_70thffs = 1
